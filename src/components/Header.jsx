...
const Header = ({ y }) => {
  ...
  return (
    <header className={`fixed top-0 w-full z-30 transition-all ${(y < -90 || pathname !== "/") ? "bg-amber-400/70 backdrop-blur-md" : ""}`}>
      <div className="container mx-auto flex p-4 h-32">
        <Link to='/' className="brand font-black flex items-center text-4xl text-white">GAME <span className={` pl-1 ${(y < -90 || pathname !== "/") ? "text-purple-600" : "text-amber-400"}`}> ON</span></Link>
